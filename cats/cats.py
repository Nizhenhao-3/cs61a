...
def accuracy(typed, source):
    ...
    if  typed==''and source=='':
        ...
        lenth=min(len(typed_words),len(source_words))
...
def wpm(typed, elapsed):
    ...
    if typed=='':
        ...
        num_typed=len(typed)
        return num_typed/5.0*(60.0/elapsed)
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for word in word_list:
        if typed_word==word:
            return typed_word
        else:diff_score.append(diff_function(typed_word,word,limit))
    min_score=min(diff_score,key=abs)
    min_score_index=diff_score.index(min(diff_score,key=abs))
    if min_score>limit:
        return typed_word
    else:return word_list[min_score_index]
    # END PROBLEM 5
...
def feline_fixes(typed, source, limit):
    ...
    #         return diff +len(typed)+len(source)
    #     if typed[0]!=source[0]:
    ...
    else:return 1+feline_fixes(typed[1:],source[1:],limit-1)
...
def report_progress(typed, source, user_id, upload):
    ...
    score=correct_num/len(source)
    infomation={'id':user_id,'progress':score}
    upload(infomation)
    return score
    # END PROBLEM 8
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    m=len(timestamps_per_player)
    n=len(words)
    ecah_player_time=[[None for i in range(n)]for j in range(m)]
    for i in range(m):
        for j in range(n):
            ecah_player_time[i][j]=timestamps_per_player[i][j+1]-timestamps_per_player[i][j]

    return match(words,ecah_player_time)
